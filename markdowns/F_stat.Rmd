...
Hypothesis 1 could not be verified: Tesla-Stock-Movements do not differ in dependency of Elon Musks relevant Tweets (t = -0.031, ns)

```{r}
t.test(movement_relevant_tweet_tesla, movement_no_relevant_tweet_tesla, var.equal = T, alternative = "two.sided")
```

creating values in order to plot the results of the indepentend sampled t-test
```{r}
# Means and create SEM
...
```{r}
#create sample data
tesla_stock_move <- data.frame(name=c("movement_relevant_tweet_tesla","movement_no_relevant_tweet_tesla"),
...
#create barplot 
ggplot(bitcoin_move, aes(name, value))+
  geom_col(size=5)+
  geom_errorbar( aes(x=name, ymin=value-SEM, ymax=value+SEM), width=0.5, colour="orange", alpha=0.9, size=0.5)
...
```{r}
#create sample data
bitcoin_move <- data.frame(name=c("movement_relevant_tweet_bitcoin","movement_no_relevant_tweet_bitcoin"),
